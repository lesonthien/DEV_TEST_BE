	...
	IF @orderId LIKE 'CTV_RECHARGE.%'
      BEGIN
	      ...
	      DECLARE @result INT;
	       ...
	       SET @actualOrderId = SUBSTRING(
		        @orderId,
		        CHARINDEX('.', @orderId) + 1,
		        LEN(@orderId) - CHARINDEX('.', @orderId)
		    )
        -- Call proc_createRecharge
                DECLARE @user_id NVARCHAR(255)
                DECLARE @note NVARCHAR(MAX)
                DECLARE @amount DECIMAL(18, 2)

                -- Retrieve values for the procedure parameters
                SELECT
                    @user_id = customer_id,
                    @note = note,
                    @amount = total_price
                ...
                EXEC @result = [dbo].[proc_createRecharge]
                    @user_id = @user_id,
                    @note = @note,
                    @amount = @amount,
                    @created_by = @user_id,
				...
				BEGIN
					...
					update forfun_order set status='COMPLETED' where id=@actualOrderId
				    -- Add additional handling logic if needed
				END
				ELSE IF @result = -1
				BEGIN
				   update forfun_order set status='ERROR_CREAT_RECHARGE' where id=@actualOrderId
				    -- Add additional error handling logic if needed
				END
				ELSE
				BEGIN
				   update forfun_order set status='ERROR_CREAT_RECHARGE' where id=@actualOrderId
				    -- Add additional handling for unexpected results if needed
				END
